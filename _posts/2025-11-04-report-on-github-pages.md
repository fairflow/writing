...
in that environment of course, and also setting up a local server means that changes can be seen really quickly (for a small site such as this, less than a second after saving!); also a local branch can be kept which has sensitive data and only after filtering would these files be pushed to a public branch.  So much cleaner that a MAMP which is not pleasant to maintain or set up in the first place.  I wanted a lightweight solution that used standard Markdown syntax to speed up writing and for compatibility with Perplexity which I use so much.

## What's not so great

Integration with Perplexity which was my main aim in setting this system up is not perfect. When on the Web the editing features of GitHub are rather limited and do not integrate with tools such as `new_post.py` which can be built locally. On the other hand, when using GitHub on the Web, Copilot is powerful for development and sits there right with the entire codebase (except private sections).
